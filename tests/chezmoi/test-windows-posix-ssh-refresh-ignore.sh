...
windows_gitea_migration=".chezmoiscripts/windows/migrate-gitea-external-remotes.ps1"
...
assert_not_ignored "$windows_ignored" "$windows_gitea_migration" windows
...
for os in darwin linux; do
  ...
  assert_not_ignored "$ignored" "$posix_gitea_migration" "$os"
  ...
  assert_ignored "$ignored" "$windows_gitea_migration" "$os"
done
