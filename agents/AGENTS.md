...
- Run relevant verification before claiming work is complete. For TS/JS changed since main: `fallow audit --changed-since main` is the primary static-analysis check.
- Use Conventional Commits; keep commit bodies minimal unless the user asks or context is essential.

## Pi Config Changes

- Durable Pi config includes: `settings.json`, `mcp.json`, `extensions/`, `npm/package*.json`, `docs/`, `AGENTS.md`, `prompts/`, `agents/`.
...
- Before editing Pi instructions, confirm `AGENTS.md` is Pi-local and not a symlink to `/Users/brendan/.claude/CLAUDE.md`.
