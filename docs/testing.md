...
- **`policy-lint` on Ubuntu 24.04** - validates the workflow with actionlint, then scopes ShellCheck, YAML, TOML, JSON, and policy checks to `.github/`, `home/`, `tests/`, `install.sh`, and `agents/.skill-lock.json`. Skill assets under `agents/skills/` are scanned separately by SkillSpector when distributed.
